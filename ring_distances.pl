...
while ($file = readdir(DH)) {
	...
	while($line = readline(FH)) {
		@params = split(/\s+/, $line);
		if ($params[3] eq "PHE" || $params[3] eq "TYR" ||$params[3] eq "TRP" || $params[3] eq "PRO" ||$params[3] eq "ILE" || $params[3] eq "GLY") {
			...
			while ($line = readline(FH)) {
				...
				if ($params2[3] eq "PHE" || $params2[3] eq "TYR" ||$params2[3] eq "TRP" || $params2[3] eq "PRO" ||$params2[3] eq "ILE" || $params2[3] eq "GLY") {
